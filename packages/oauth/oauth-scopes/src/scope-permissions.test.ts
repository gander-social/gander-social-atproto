  ...
  describe('allowsRpc', () => {
    it('should ignore "rpc:*?lxm=*"', () => {
      ...
      expect(
        set.allowsRpc({
          aud: 'did:web:example.com',
          lxm: 'app.gndr.feed.getFeed',
        }),
      ).toBe(false)
    })

    it('should allow constraining "lxm"', () => {
      const set = new ScopePermissions('rpc:app.gndr.feed.getFeed?aud=*')
      expect(
        set.allowsRpc({
          aud: 'did:web:example.com',
          lxm: 'app.gndr.feed.getFeed',
      ...
      expect(
        set.allowsRpc({ aud: 'did:plc:blahbla', lxm: 'app.gndr.feed.getFeed' }),
      ).toBe(true)
    ...
    it('should allow constraining "lxm" and "aud"', () => {
      const set = new ScopePermissions(
        'rpc:app.gndr.feed.getFeed?aud=did:web:example.com%23foo',
      )
      expect(
        set.allowsRpc({
          aud: 'did:web:example.com#foo',
    ...
    it('should ignore "transition:chat.gndr"', () => {
      const set = new ScopePermissions('transition:chat.gndr')
      expect(
        set.allowsRpc({
          aud: 'did:web:example.com',
