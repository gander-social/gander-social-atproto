...
import { OutputSchema as GetListsOutputSchema } from '../../src/lexicon/types/app/gndr/graph/getLists'
import {
  ListWithMembership,
  OutputSchema as GetListsWithMembershipOutputSchema,
} from '../../src/lexicon/types/app/gndr/graph/getListsWithMembership'
...
describe('gndr actor likes feed views', () => {
  ...
  it('allows filtering by list purpose', async () => {
    const viewCurate = await agent.app.gndr.graph.getLists({
      actor: eve,
      purposes: ['curatelist'],
    })
    expect(viewCurate.data.lists.length).toBe(3)

    const viewMod = await agent.app.gndr.graph.getLists({
      actor: eve,
      purposes: ['modlist'],
    })
    expect(viewMod.data.lists.length).toBe(3)

    const viewAll = await agent.app.gndr.graph.getLists({
      actor: eve,
  ...
  it.each([
    ...
    async ({ expected, purposes }) => {
      ...
      const full = await agent.app.gndr.graph.getLists(
        { actor: eve, purposes },
        {
          headers: await network.serviceHeaders(eve, ids.AppGndrGraphGetLists),
        },
      )
  ...
  it('does not include users with creator block relationship in reference lists for non-creator, in-list viewers', async () => {
    const curView = await agent.app.gndr.graph.getList(
      {
        list: curateList,
      },
      {
    ...
    const refView = await agent.app.gndr.graph.getList(
      { list: referenceList },
      {
        headers: await network.serviceHeaders(frankie, ids.AppGndrGraphGetList),
      },
  ...
  it('does return all users regardless of creator block relationship in moderation lists', async () => {
    const view = await agent.app.gndr.graph.getList(
      { list: blockList },
  ...
  describe('list membership', () => {
    ...
    it('returns all lists by the user', async () => {
      const view = await agent.app.gndr.graph.getListsWithMembership(
        { actor: frankie },
        {
          headers: await network.serviceHeaders(
            eve,
            ids.AppGndrGraphGetListsWithMembership,
          ),
        },
      )
    ...
    it.each([
      ...
      async ({ expected, purposes }) => {
        ...
        const paginator = async (cursor?: string) => {
          const res = await agent.app.gndr.graph.getListsWithMembership(
            { actor: eve, purposes, limit: 2, cursor },
            {
              headers: await network.serviceHeaders(
                eve,
        ...
        const full = await agent.app.gndr.graph.getListsWithMembership(
          { actor: eve, purposes },
          {
            headers: await network.serviceHeaders(
              eve,
