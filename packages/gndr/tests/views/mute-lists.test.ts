...
describe('gndr views with mutes from mute lists', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_views_mute_lists',
    })
  ...
  it('flags muted accounts in get suggestions', async () => {
    ...
    const res = await agent.api.app.gndr.actor.getSuggestions(
      {
        limit: 100,
      },
      {
  ...
  it('returns a users own list mutes', async () => {
    ...
    const res = await agent.api.app.gndr.graph.getListMutes(
      {},
      {
        headers: await network.serviceHeaders(
          dan,
  ...
  it('paginates getListMutes', async () => {
    const full = await agent.api.app.gndr.graph.getListMutes(
      {},
      {
        headers: await network.serviceHeaders(
          dan,
    ...
    const first = await agent.api.app.gndr.graph.getListMutes(
      { limit: 1 },
      {
        headers: await network.serviceHeaders(
          dan,
    ...
    const second = await agent.api.app.gndr.graph.getListMutes(
