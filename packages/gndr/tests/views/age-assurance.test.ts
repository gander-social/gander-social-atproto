...
describe('age assurance views', () => {
  ...
  const redirectUrl = 'https://gndr.app/intent/age-assurance'
  ...
  beforeAll(async () => {
    ...
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_views_age_assurance',
      gndr: {
        statsigEnv: 'test',
    ...
    const { data } = await agent.app.gndr.unspecced.getAgeAssuranceState(
      {},
      {
        headers: await network.serviceHeaders(
          did,
          ids.AppGndrUnspeccedGetAgeAssuranceState,
        ),
      },
    )
    return data
  ...
  const initAgeAssurance = async (did: string, email?: string) => {
    const { data } = await agent.app.gndr.unspecced.initAgeAssurance(
      {
      ...
      },
      {
        headers: await network.serviceHeaders(
          did,
          ids.AppGndrUnspeccedInitAgeAssurance,
        ),
      },
    )
    return data
  ...
  it('ensures user cannot re-init flow from terminal state', async () => {
    ...
    await kwsServer.callVerificationResponse(network.gndr.url, {
      externalPayload,
      status,
    })
  ...
  describe('webhook flow', () => {
    it('performs the AA flow', async () => {
      ...
      const webhookRes = await kwsServer.callWebhook(network.gndr.url, {
        payload: {
          externalPayload: {
            actorDid,
            attemptId,
...
class MockKwsServer {
  ...
  callVerificationResponse(
    ...
    return fetch(
      `${gndrUrl}/external/kws/age-assurance-verification?${queryString}`,
      {
        method: 'GET',
        redirect: 'manual',
      },
  ...
  callWebhook(gndrUrl: string, body: KwsWebhookBody): Promise<Response> {
    ...
    return fetch(`${gndrUrl}/external/kws/age-assurance-webhook`, {
      method: 'POST',
      body: bodyBuffer,
      headers: {
