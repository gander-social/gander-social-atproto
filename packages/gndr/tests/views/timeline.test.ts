...
import { OutputSchema as GetTimelineOutputSchema } from '../../src/lexicon/types/app/gndr/feed/getTimeline'
...
describe('timeline views', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_views_home_feed',
    })
  ...
  // @TODO(gndr) blocks posts, reposts, replies by actor takedown via labels
  // @TODO(gndr) blocks posts, reposts, replies by record takedown via labels
  ...
  it("fetches authenticated user's home feed w/ reverse-chronological algorithm", async () => {
    ...
    const aliceTL = await agent.api.app.gndr.feed.getTimeline(
    ...
    const carolTL = await agent.api.app.gndr.feed.getTimeline(
      { algorithm: REVERSE_CHRON },
      {
        headers: await network.serviceHeaders(
          carol,
  ...
  it("fetches authenticated user's home feed w/ default algorithm", async () => {
    const defaultTL = await agent.api.app.gndr.feed.getTimeline(
      {},
      {
        headers: await network.serviceHeaders(
          alice,
    ...
    const reverseChronologicalTL = await agent.api.app.gndr.feed.getTimeline(
      { algorithm: REVERSE_CHRON },
      {
        headers: await network.serviceHeaders(
          alice,
  ...
  it('paginates reverse-chronological feed', async () => {
    ...
    const full = await agent.api.app.gndr.feed.getTimeline(
      {
        algorithm: REVERSE_CHRON,
      },
      {
  ...
  it('agrees what the first item is for limit=1 and other limits', async () => {
    const { data: timeline } = await agent.api.app.gndr.feed.getTimeline(
      { limit: 10 },
      {
        headers: await network.serviceHeaders(
          alice,
    ...
    const { data: timelineLimit1 } = await agent.api.app.gndr.feed.getTimeline(
      { limit: 1 },
      {
        headers: await network.serviceHeaders(
          alice,
  ...
  it('blocks posts, reposts, replies by actor takedown', async () => {
    await Promise.all(
      [bob, carol].map((did) =>
        network.gndr.ctx.dataplane.takedownActor({ did }),
      ),
    )
    ...
    await Promise.all(
      [bob, carol].map((did) =>
        network.gndr.ctx.dataplane.untakedownActor({ did }),
      ),
    )
  })
