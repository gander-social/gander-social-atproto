...
describe('pds repost views', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_views_reposts',
    })
  ...
  it('fetches reposted-by for a post', async () => {
    const view = await agent.api.app.gndr.feed.getRepostedBy(
  ...
  it('paginates', async () => {
    ...
    const full = await agent.api.app.gndr.feed.getRepostedBy(
      { uri: sc.posts[alice][2].ref.uriStr },
      {
        headers: await network.serviceHeaders(
          alice,
  ...
  it('fetches reposted-by unauthed', async () => {
    const { data: authed } = await agent.api.app.gndr.feed.getRepostedBy(
      { uri: sc.posts[alice][2].ref.uriStr },
      {
        headers: await network.serviceHeaders(
          alice,
    ...
    const { data: unauthed } = await agent.api.app.gndr.feed.getRepostedBy({
      uri: sc.posts[alice][2].ref.uriStr,
    })
