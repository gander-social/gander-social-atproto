...
describe('pds posts views', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_views_posts',
    })
  ...
  it('fetches posts', async () => {
    ...
    const posts = await agent.api.app.gndr.feed.getPosts(
      { uris },
      {
  ...
  it('fetches posts unauthed', async () => {
    ...
    const authed = await agent.api.app.gndr.feed.getPosts(
      { uris },
      {
        headers: await network.serviceHeaders(
          sc.dids.alice,
    ...
    const unauthed = await agent.api.app.gndr.feed.getPosts({
      uris,
    })
  ...
  it('handles repeat uris', async () => {
    ...
    const posts = await agent.api.app.gndr.feed.getPosts({ uris })
  ...
  it('allows for creating posts with tags', async () => {
    ...
    const { uri } = await pdsAgent.api.app.gndr.feed.post.create(
      { repo: sc.dids.alice },
      post,
    ...
    const { data } = await agent.api.app.gndr.feed.getPosts({ uris: [uri] })
  ...
  it('embeds video.', async () => {
    ...
    await network.processAll()
    const { data } = await agent.app.gndr.feed.getPosts({ uris: [uri] })
    expect(data.posts.length).toBe(1)
  ...
  it('embeds video with record.', async () => {
    ...
    const embedRecord = await pdsAgent.api.app.gndr.feed.post.create(
      { repo: sc.dids.alice },
      {
        text: 'embedded',
        createdAt: new Date().toISOString(),
