...
describe.skip('pds actor search views', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_views_actor_search',
    })
  ...
  it('typeahead gives relevant results', async () => {
    const result = await agent.api.app.gndr.actor.searchActorsTypeahead(
      { term: 'car' },
      { headers },
    )
  ...
  it('typeahead applies limit', async () => {
    const full = await agent.api.app.gndr.actor.searchActorsTypeahead(
      { term: 'p' },
      { headers },
    )
    ...
    const limited = await agent.api.app.gndr.actor.searchActorsTypeahead(
      { term: 'p', limit: 5 },
      { headers },
    )
  ...
  it('typeahead gives results unauthed', async () => {
    const { data: authed } =
      await agent.api.app.gndr.actor.searchActorsTypeahead(
        { term: 'car' },
        { headers },
      )
    const { data: unauthed } =
  ...
  it('search gives relevant results', async () => {
    const result = await agent.api.app.gndr.actor.searchActors(
      { term: 'car' },
      { headers },
    )
  ...
  it('paginates', async () => {
    ...
    const paginator = async (cursor?: string) => {
      const res = await agent.api.app.gndr.actor.searchActors(
    ...
    const full = await agent.api.app.gndr.actor.searchActors(
      { term: 'p' },
      { headers },
    )
  ...
  it('search gives results unauthed', async () => {
    const { data: authed } = await agent.api.app.gndr.actor.searchActors(
      { term: 'car' },
      { headers },
    )
    const { data: unauthed } = await agent.api.app.gndr.actor.searchActors({
      term: 'car',
    })
    expect(unauthed.actors.length).toBeGreaterThan(0)
    expect(unauthed.actors).toEqual(authed.actors.map(stripViewer))
  ...
  it('search blocks by actor takedown', async () => {
    await network.gndr.server.ctx.dataplane.takedownActor({
