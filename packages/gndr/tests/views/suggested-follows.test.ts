...
import { SeedClient, TestNetwork, likesSeed } from '@gander-social-atproto/dev-env'
...
describe('suggested follows', () => {
  ...
  it('returns sorted suggested follows for carol', async () => {
    const result = await agent.api.app.gndr.graph.getSuggestedFollowsByActor(
      {
        actor: sc.dids.alice,
      },
      {
  ...
  it('exludes users muted by viewer', async () => {
    await pdsAgent.api.app.gndr.graph.muteActor(
      { actor: sc.dids.bob },
  ...
  it('exludes users blocked by viewer', async () => {
    const carolBlocksBob = await pdsAgent.api.app.gndr.graph.block.create(
      { repo: sc.dids.carol },
  ...
  it('exludes users blocking viewer', async () => {
    const bobBlocksCarol = await pdsAgent.api.app.gndr.graph.block.create(
      { repo: sc.dids.bob },
