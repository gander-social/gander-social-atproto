...
describe('known followers (social proof)', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_known_followers',
    })
  ...
  it('basic profile views do not return knownFollowers', async () => {
    const { data } = await agent.api.app.gndr.graph.getFollows(
  ...
  it('getKnownFollowers: returns data', async () => {
    const { data } = await agent.api.app.gndr.graph.getKnownFollowers(
      { actor: dids.base_sub },
      {
        headers: await network.serviceHeaders(
          dids.base_view,
  ...
  it('getProfile: returns knownFollowers', async () => {
    const { data } = await agent.api.app.gndr.actor.getProfile(
      { actor: dids.base_sub },
      {
        headers: await network.serviceHeaders(
          dids.base_view,
  ...
  it('getProfile: filters second-party blocks', async () => {
    const result = await agent.api.app.gndr.actor.getProfile(
  ...
  it('getProfiles: filters second-party blocks', async () => {
    const result = await agent.api.app.gndr.actor.getProfiles(
