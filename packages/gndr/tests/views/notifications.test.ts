import { AppGndrNotificationDeclaration, AtpAgent } from '@gander-social-atproto/api'
...
describe('notification views', () => {
  ...
  it('fetches notification count without a last-seen', async () => {
    const notifCountAlice =
      await agent.api.app.gndr.notification.getUnreadCount(
        {},
        {
    ...
    const notifCountBob = await agent.api.app.gndr.notification.getUnreadCount(
      {},
      {
        headers: await network.serviceHeaders(
          sc.dids.bob,
  ...
  it('does not give notifs for a deleted subject', async () => {
    ...
    const notifsAlice = await agent.api.app.gndr.notification.listNotifications(
      {},
      {
        headers: await network.serviceHeaders(
          sc.dids.alice,
  ...
  it('generates notifications for quotes', async () => {
    ...
    const notifsDan = await agent.api.app.gndr.notification.listNotifications(
      {},
      {
        headers: await network.serviceHeaders(
          sc.dids.dan,
  ...
  it('generates notifications for likes', async () => {
    const notifsAlice = await agent.app.gndr.notification.listNotifications(
      {},
      {
        headers: await network.serviceHeaders(
          alice,
  ...
  it('generates notifications for likes via repost', async () => {
    ...
    const notifsOp = await agent.app.gndr.notification.listNotifications(
      {},
      {
        headers: await network.serviceHeaders(
          op,
    ...
    const notifsReposter = await agent.app.gndr.notification.listNotifications(
      {},
      {
        headers: await network.serviceHeaders(
          reposter,
  ...
  it('generates notifications for verification created and removed', async () => {
    ...
    await network.processAll()
    const notifsBob1 = await agent.app.gndr.notification.listNotifications(
    ...
    await network.processAll()
    const notifsBob2 = await agent.app.gndr.notification.listNotifications(
      { reasons: ['verified', 'unverified'] },
      {
        headers: await network.serviceHeaders(
          sc.dids.bob,
  ...
  it('fetches notifications without a last-seen', async () => {
    const notifRes = await agent.api.app.gndr.notification.listNotifications(
      {},
      {
        headers: await network.serviceHeaders(
          alice,
  ...
  it('paginates', async () => {
    ...
    const full = await agent.api.app.gndr.notification.listNotifications(
      {},
      {
        headers: await network.serviceHeaders(
          alice,
  ...
  it('fetches notification count with a last-seen', async () => {
    ...
    const full2 = await agent.api.app.gndr.notification.listNotifications(
      {},
      {
        headers: await network.serviceHeaders(
          alice,
    ...
    const notifCount = await agent.api.app.gndr.notification.getUnreadCount(
      {},
      {
        headers: await network.serviceHeaders(
          alice,
  ...
  it('fetches notifications omitting mentions and replies for taken-down posts', async () => {
    ...
    await Promise.all(
      [postRef1, postRef2].map((postRef) =>
        network.gndr.ctx.dataplane.takedownRecord({
    ...
    await Promise.all(
      [postRef1, postRef2].map((postRef) =>
        network.gndr.ctx.dataplane.untakedownRecord({
          recordUri: postRef.uriStr,
        }),
      ),
    )
  ...
  it('fetches notifications with explicit priority', async () => {
    const priority = await agent.api.app.gndr.notification.listNotifications(
      { priority: true },
      {
        headers: await network.serviceHeaders(
          sc.dids.carol,
    ...
    const noPriority = await agent.api.app.gndr.notification.listNotifications(
      { priority: false },
      {
        headers: await network.serviceHeaders(
          sc.dids.carol,
  ...
  it('fetches notifications with default priority', async () => {
    await agent.api.app.gndr.notification.putPreferences(
      { priority: true },
      {
        encoding: 'application/json',
        headers: await network.serviceHeaders(
    ...
    await network.processAll()
    const notifs = await agent.api.app.gndr.notification.listNotifications(
      {},
      {
        headers: await network.serviceHeaders(
          sc.dids.carol,
  ...
  it('paginates filtered notifications', async () => {
    ...
    const full = await agent.app.gndr.notification.listNotifications(
      { reasons: ['mention', 'reply'] },
      {
        headers: await network.serviceHeaders(
          alice,
  ...
  describe('notifications delay', () => {
    ...
    beforeAll(async () => {
      delayNetwork = await TestNetwork.create({
        ...
        dbPostgresSchema: 'gndr_views_notifications_delay',
      })
      delayAgent = delayNetwork.gndr.getClient()
    ...
    it('paginates', async () => {
      const firstNotification = await delayNetwork.gndr.db.db
      ...
      const paginator = async (cursor?: string) => {
        const res =
          await delayAgent.api.app.gndr.notification.listNotifications(
            { cursor, limit: 6 },
            {
      ...
      const lastNotification = await delayNetwork.gndr.db.db
        .selectFrom('notification')
        .selectAll()
        .limit(1)
        .orderBy('sortAt', 'desc')
  ...
  describe('preferences v2', () => {
    ...
    it('gets preferences filling up with the defaults', async () => {
      ...
      const getAndAssert = async (
        ...
        const { data } = await agent.app.gndr.notification.getPreferences(
          {},
          {
            headers: await network.serviceHeaders(
              actorDid,
      ...
      await agent.app.gndr.notification.putPreferencesV2(
    ...
    it('stores the preferences setting the defaults', async () => {
      ...
      const putAndAssert = async (
        ...
        const { data } = await agent.app.gndr.notification.putPreferencesV2(
          input,
          {
            encoding: 'application/json',
            headers: await network.serviceHeaders(
  ...
  describe('activity subscriptions', () => {
    ...
    const put = async (
      ...
      agent.app.gndr.notification.putActivitySubscription(
        {
          subject,
    ...
    const associatedAllowSub = async (actor: string, subject: string) => {
      const { data } = await agent.app.gndr.actor.getProfile(
        { actor: subject },
        {
          headers: await network.serviceHeaders(
            actor,
