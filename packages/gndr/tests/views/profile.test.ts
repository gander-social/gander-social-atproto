...
describe('pds profile views', () => {
  ...
  it('fetches own profile', async () => {
    const aliceForAlice = await agent.api.app.gndr.actor.getProfile(
      { actor: alice },
      {
  ...
  it('fetches multiple profiles', async () => {
    const {
      data: { profiles },
    } = await agent.api.app.gndr.actor.getProfiles(
      {
        actors: [
          alice,
          'bob.test',
  ...
  it('fetches multiple profiles unauthed', async () => {
    const { data: authed } = await agent.api.app.gndr.actor.getProfiles(
      {
