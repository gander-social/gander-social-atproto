...
import { SeedClient, TestNetwork, authorFeedSeed } from '@gander-social-atproto/dev-env'
...
describe('pds author feed views', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_views_author_feed',
    })
  ...
  // @TODO(gndr) blocked by actor takedown via labels.
  // @TODO(gndr) blocked by record takedown via labels.

  it('fetches full author feeds for self (sorted, minimal viewer state).', async () => {
    const aliceForAlice = await agent.api.app.gndr.feed.getAuthorFeed(
    ...
    const bobForBob = await agent.api.app.gndr.feed.getAuthorFeed(
    ...
    const carolForCarol = await agent.api.app.gndr.feed.getAuthorFeed(
    ...
    const danForDan = await agent.api.app.gndr.feed.getAuthorFeed(
  ...
  it("reflects fetching user's state in the feed.", async () => {
    const aliceForCarol = await agent.api.app.gndr.feed.getAuthorFeed(
      { actor: sc.accounts[alice].handle },
      {
        headers: await network.serviceHeaders(
          carol,
  ...
  it('paginates', async () => {
    ...
    const full = await agent.api.app.gndr.feed.getAuthorFeed(
      { actor: sc.accounts[alice].handle },
      {
        headers: await network.serviceHeaders(
          dan,
  ...
  it('fetches results unauthed.', async () => {
    const { data: authed } = await agent.api.app.gndr.feed.getAuthorFeed(
      { actor: sc.accounts[alice].handle },
      {
        headers: await network.serviceHeaders(
          alice,
    ...
    const { data: unauthed } = await agent.api.app.gndr.feed.getAuthorFeed({
      actor: sc.accounts[alice].handle,
    })
  ...
  it('non-admins blocked by actor takedown.', async () => {
    const { data: preBlock } = await agent.api.app.gndr.feed.getAuthorFeed(
      { actor: alice },
      {
        headers: await network.serviceHeaders(
          carol,
    ...
    const attemptAsUser = agent.api.app.gndr.feed.getAuthorFeed(
      { actor: alice },
      {
        headers: await network.serviceHeaders(
          carol,
    ...
    const attemptAsAdmin = await agent.api.app.gndr.feed.getAuthorFeed(
      { actor: alice },
  ...
  it('blocked by record takedown.', async () => {
    ...
    const [{ data: postBlockAsUser }, { data: postBlockAsAdmin }] =
      await Promise.all([
        agent.api.app.gndr.feed.getAuthorFeed(
          { actor: alice },
          {
            headers: await network.serviceHeaders(
              carol,
    ...
    // Cleanup
    await network.gndr.ctx.dataplane.untakedownRecord({
      recordUri: post.uri,
    })
  })

  it('can filter by posts_with_media', async () => {
    const { data: carolFeed } = await agent.api.app.gndr.feed.getAuthorFeed({
      actor: carol,
    ...
    const { data: bobFeed } = await agent.api.app.gndr.feed.getAuthorFeed({
      actor: bob,
      filter: 'posts_with_media',
    })
    ...
    const { data: danFeed } = await agent.api.app.gndr.feed.getAuthorFeed({
      actor: dan,
      filter: 'posts_with_media',
    })
  ...
  it('can filter by posts_with_video', async () => {
    const { data: carolFeedBefore } =
      await agent.api.app.gndr.feed.getAuthorFeed({
        actor: carol,
  ...
  it('posts_and_author_threads includes self-replies', async () => {
    const { data: eveFeed } = await agent.api.app.gndr.feed.getAuthorFeed({
      actor: eve,
  ...
  describe('pins', () => {
    ...
    it('params.includePins = true, pin is in first page of results', async () => {
      ...
      await sc.post(alice, 'not pinned post')
      await network.processAll()
      const { data } = await agent.api.app.gndr.feed.getAuthorFeed(
    ...
    it('params.includePins = true, pin is NOT in first page of results', async () => {
      ...
      await sc.post(alice, 'not pinned post')
      await network.processAll()
      const { data: page1 } = await agent.api.app.gndr.feed.getAuthorFeed(
      ...
      const { data: page2 } = await agent.api.app.gndr.feed.getAuthorFeed(
        {
          actor: sc.accounts[alice].handle,
          includePins: true,
          cursor: page1.cursor,
