...
} from '../../src/lexicon/types/app/gndr/graph/getStarterPacksWithMembership'
...
describe('starter packs', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_views_starter_packs',
    })
  ...
  it('gets actor starter packs.', async () => {
    const { data } = await agent.api.app.gndr.graph.getActorStarterPacks({
      actor: sc.dids.alice,
    })
  ...
  it('gets starter pack details', async () => {
    const {
      data: { starterPacks },
    } = await agent.api.app.gndr.graph.getStarterPacks({
  ...
  it('generates notifications', async () => {
    ...
    notifications.forEach((notif) => {
      ...
      expect(notif.uri).toMatch(/\/app\.gndr\.actor\.profile\/self$/)
  ...
  it('does not include users with creator block relationship in list sample for non-creator, in-list viewers', async () => {
    const view = await agent.api.app.gndr.graph.getStarterPack(
      {
  ...
  describe('searchStarterPacks', () => {
    it('searches starter packs and returns paginated', async () => {
      const { data: page0 } = await agent.app.gndr.graph.searchStarterPacks({
        q: 'starter',
        limit: 3,
      })
      ...
      const { data: page1 } = await agent.api.app.gndr.graph.searchStarterPacks(
        {
          q: 'starter',
          limit: 3,
          cursor: page0.cursor,
    ...
    it('filters by the search term', async () => {
      const { data } = await agent.app.gndr.graph.searchStarterPacks({
        q: 'In CaSe',
        limit: 3,
      })
  ...
  describe('starter pack membership', () => {
    ...
    it('returns all SPs by the user', async () => {
      const view = await agent.app.gndr.graph.getStarterPacksWithMembership(
        { actor: sc.dids.bob },
        {
          headers: await network.serviceHeaders(
            sc.dids.alice,
            ids.AppGndrGraphGetStarterPacksWithMembership,
          ),
        },
      )
    ...
    it('paginates SPs with memberships', async () => {
      ...
      const paginator = async (cursor?: string) => {
        const res = await agent.app.gndr.graph.getStarterPacksWithMembership(
      ...
      const full = await agent.app.gndr.graph.getStarterPacksWithMembership(
        { actor },
        {
          headers: await network.serviceHeaders(
            viewer,
