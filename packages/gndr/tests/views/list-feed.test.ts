...
describe('list feed views', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_views_list_feed',
    })
  ...
  it('paginates', async () => {
    ...
    const full = await agent.api.app.gndr.feed.getListFeed(
      { list: listRef.uriStr },
      {
        headers: await network.serviceHeaders(
          carol,
  ...
  it('fetches results unauthed', async () => {
    const { data: authed } = await agent.api.app.gndr.feed.getListFeed(
      { list: listRef.uriStr },
      {
        headers: await network.serviceHeaders(
          alice,
    ...
    const { data: unauthed } = await agent.api.app.gndr.feed.getListFeed({
      list: listRef.uriStr,
    })
