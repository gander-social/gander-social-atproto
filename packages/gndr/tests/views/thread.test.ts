...
describe('appview thread views', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_views_thread',
    })
  ...
  it('fetches deep post thread', async () => {
    const thread = await agent.api.app.gndr.feed.getPostThread(
  ...
  it('fails for an unknown post', async () => {
    const promise = agent.api.app.gndr.feed.getPostThread(
  ...
  it('fetches post thread unauthed', async () => {
    const { data: authed } = await agent.api.app.gndr.feed.getPostThread(
      { uri: sc.posts[alice][1].ref.uriStr },
      {
        headers: await network.serviceHeaders(
          bob,
    ...
    const { data: unauthed } = await agent.api.app.gndr.feed.getPostThread({
      uri: sc.posts[alice][1].ref.uriStr,
    })
  ...
  it('handles deleted posts correctly', async () => {
    ...
    const thread1 = await agent.api.app.gndr.feed.getPostThread(
    ...
    const thread2 = await agent.api.app.gndr.feed.getPostThread(
      { uri: sc.posts[alice][indexes.aliceRoot].ref.uriStr },
      {
        headers: await network.serviceHeaders(
          bob,
    ...
    const thread3 = await agent.api.app.gndr.feed.getPostThread(
  ...
  describe('takedown', () => {
    ...
    it('blocks ancestors by record', async () => {
      const threadPreTakedown = await agent.api.app.gndr.feed.getPostThread(
        { depth: 1, uri: sc.replies[alice][0].ref.uriStr },
        {
          headers: await network.serviceHeaders(
            bob,
  ...
  describe('listblock', () => {
    it(`doesn't apply listblock if list was taken down by private takedown`, async () => {
      ...
      const listblockRef = await pdsAgent.app.gndr.graph.listblock.create(
        { repo: bob },
        {
      ...
      const threadBeforeListTakedown = await agent.app.gndr.feed.getPostThread(
      ...
      const threadAfterListTakedown = await agent.app.gndr.feed.getPostThread(
        { depth: 1, uri: reply.ref.uriStr },
        {
          headers: await network.serviceHeaders(
            carol,
      ...
      // Cleanup
      await pdsAgent.app.gndr.graph.listblock.delete(
    ...
    it(`doesn't apply listblock if list was taken down by takedown label`, async () => {
      ...
      await network.gndr.db.db.insertInto('label').values(label).execute()
