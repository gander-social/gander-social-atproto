...
describe('pds views with blocking', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_views_block',
    })
    ...
    // dan blocks carol
    danBlockCarol = await pdsAgent.api.app.gndr.graph.block.create(
      { repo: dan },
  ...
  it('strips blocked users out of getListFeed', async () => {
    ...
    const resCarol = await agent.api.app.gndr.feed.getListFeed(
  ...
  it('does not return block violating follows', async () => {
    const resCarol = await agent.api.app.gndr.graph.getFollows(
      { actor: carol },
      {
        headers: await network.serviceHeaders(
          alice,
    ...
    const resDan = await agent.api.app.gndr.graph.getFollows(
      { actor: dan },
      {
        headers: await network.serviceHeaders(
          alice,
  ...
  it('does not return block violating followers', async () => {
    const resCarol = await agent.api.app.gndr.graph.getFollowers(
      { actor: carol },
      {
        headers: await network.serviceHeaders(
          alice,
    ...
    const resDan = await agent.api.app.gndr.graph.getFollowers(
      { actor: dan },
      {
        headers: await network.serviceHeaders(
          alice,
  ...
  it('does not serve blocked replies', async () => {
    ...
    await network.processAll()
    const { data: unblock } = await agent.api.app.gndr.feed.getPostThread(
      { depth: 1, uri: sc.posts[dan][0].ref.uriStr },
      {
        headers: await network.serviceHeaders(
          alice,
    ...
    // cleanup
    await pdsAgent.api.app.gndr.feed.post.delete(
  ...
  it('returns a list of blocks', async () => {
    await pdsAgent.api.app.gndr.graph.block.create(
      { repo: dan },
  ...
  it('returns knownFollowers with blocks filtered', async () => {
    const carolForAlice = await agent.api.app.gndr.actor.getProfile(
      { actor: bob },
      {
        headers: await network.serviceHeaders(
          alice,
