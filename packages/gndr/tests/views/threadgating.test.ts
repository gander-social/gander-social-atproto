...
describe('views with thread gating', () => {
  ...
  it('applies gate for following rule.', async () => {
    ...
    await pdsAgent.api.app.gndr.feed.threadgate.create(
      { repo: sc.dids.carol, rkey: post.ref.uri.rkey },
      {
        post: post.ref.uriStr,
        createdAt: iso(),
        allow: [{ $type: 'app.gndr.feed.threadgate#followingRule' }],
      },
      sc.getHeaders(sc.dids.carol),
    )
    await network.processAll()
  ...
  it('applies gate for follower rule.', async () => {
    ...
    await pdsAgent.api.app.gndr.feed.threadgate.create(
      { repo: sc.dids.carol, rkey: post.ref.uri.rkey },
      {
        post: post.ref.uriStr,
        createdAt: iso(),
        allow: [{ $type: 'app.gndr.feed.threadgate#followerRule' }],
      },
      sc.getHeaders(sc.dids.carol),
    )
    await network.processAll()
  ...
  it('applies gate for list rule.', async () => {
    ...
    const listA = await pdsAgent.api.app.gndr.graph.list.create(
      { repo: sc.dids.carol },
      {
        name: 'list a',
    ...
    const listB = await pdsAgent.api.app.gndr.graph.list.create(
      { repo: sc.dids.carol },
      {
        name: 'list b',
        purpose: 'app.gndr.graph.defs#modlist',
    ...
    await pdsAgent.api.app.gndr.feed.threadgate.create(
      { repo: sc.dids.carol, rkey: post.ref.uri.rkey },
      {
        ...
        createdAt: iso(),
        allow: [
          { $type: 'app.gndr.feed.threadgate#listRule', list: listA.uri },
          { $type: 'app.gndr.feed.threadgate#listRule', list: listB.uri },
        ],
      },
      sc.getHeaders(sc.dids.carol),
    )
