...
describe('indexing', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_indexing',
    })
  ...
  it('indexes posts.', async () => {
    ...
    // Create
    await network.gndr.sub.indexingSvc.indexRecord(...createRecord)

    const getAfterCreate = await agent.api.app.gndr.feed.getPostThread(
      { uri: uri.toString() },
      {
    ...
    // Update
    await network.gndr.sub.indexingSvc.indexRecord(...updateRecord)

    const getAfterUpdate = await agent.api.app.gndr.feed.getPostThread(
      { uri: uri.toString() },
      {
        headers: await network.serviceHeaders(
          sc.dids.alice,
    ...
    // Delete
    await network.gndr.sub.indexingSvc.deleteRecord(...deleteRecord)

    const getAfterDelete = agent.api.app.gndr.feed.getPostThread(
      { uri: uri.toString() },
      {
        headers: await network.serviceHeaders(
          sc.dids.alice,
  ...
  it('indexes profiles.', async () => {
    ...
    const getAfterCreate = await agent.api.app.gndr.actor.getProfile(
      { actor: sc.dids.dan },
      {
        headers: await network.serviceHeaders(
          sc.dids.alice,
    ...
    const getAfterUpdate = await agent.api.app.gndr.actor.getProfile(
      { actor: sc.dids.dan },
      {
        headers: await network.serviceHeaders(
          sc.dids.alice,
    ...
    const getAfterDelete = await agent.api.app.gndr.actor.getProfile(
      { actor: sc.dids.dan },
      {
        headers: await network.serviceHeaders(
          sc.dids.alice,
  ...
  it('handles post aggregations out of order.', async () => {
    ...
    await network.gndr.sub.indexingSvc.indexRecord(...like)
    await network.gndr.sub.indexingSvc.indexRecord(...repost)
    ...
    await network.gndr.sub.indexingSvc.deleteRecord(...del(reply[0]))
    await network.gndr.sub.indexingSvc.deleteRecord(...del(like[0]))
    await network.gndr.sub.indexingSvc.deleteRecord(...del(repost[0]))
    await network.gndr.sub.indexingSvc.deleteRecord(...del(originalPost[0]))
  })
  ...
  it('does not notify user of own like or repost', async () => {
    ...
    await network.gndr.sub.indexingSvc.indexRecord(...originalPost)
    await network.gndr.sub.indexingSvc.indexRecord(...ownLike)
    await network.gndr.sub.indexingSvc.indexRecord(...ownRepost)
    await network.gndr.sub.indexingSvc.indexRecord(...aliceLike)
    await network.gndr.sub.indexingSvc.indexRecord(...aliceRepost)
    await network.gndr.sub.background.processAll()
    ...
    await network.gndr.sub.indexingSvc.deleteRecord(...del(ownLike[0]))
    await network.gndr.sub.indexingSvc.deleteRecord(...del(ownRepost[0]))
    await network.gndr.sub.indexingSvc.deleteRecord(...del(aliceLike[0]))
    await network.gndr.sub.indexingSvc.deleteRecord(...del(aliceRepost[0]))
    await network.gndr.sub.indexingSvc.deleteRecord(...del(originalPost[0]))
  })
  ...
  it('handles profile aggregations out of order.', async () => {
    ...
    await network.gndr.sub.indexingSvc.deleteRecord(...del(follow[0]))
  })

  describe('indexRepo', () => {
    beforeAll(async () => {
      await network.gndr.sub.restart()
    ...
    it('preserves indexes when no record changes.', async () => {
      // Mark originals
      const { data: origProfile } = await agent.api.app.gndr.actor.getProfile(
        { actor: sc.dids.alice },
        {
          headers: await network.serviceHeaders(
            sc.dids.alice,
      ...
      const { data: origFeed } = await agent.api.app.gndr.feed.getAuthorFeed(
        { actor: sc.dids.alice },
        {
          headers: await network.serviceHeaders(
            sc.dids.alice,
      ...
      const { data: origFollows } = await agent.api.app.gndr.graph.getFollows(
        { actor: sc.dids.alice },
        {
          headers: await network.serviceHeaders(
            sc.dids.alice,
      ...
      await network.gndr.sub.indexingSvc.indexRepo(sc.dids.alice, commit.cid)
      await network.gndr.sub.background.processAll()
      // Check
      ...
      const { data: feed } = await agent.api.app.gndr.feed.getAuthorFeed(
        { actor: sc.dids.alice },
        {
          headers: await network.serviceHeaders(
            sc.dids.alice,
      ...
      const { data: follows } = await agent.api.app.gndr.graph.getFollows(
        { actor: sc.dids.alice },
        {
          headers: await network.serviceHeaders(
            sc.dids.alice,
    ...
    it('updates indexes when records change.', async () => {
      ...
      await pdsAgent.api.app.gndr.graph.follow.delete(
    ...
    it('skips invalid records.', async () => {
      ...
      await network.gndr.sub.indexingSvc.indexRepo(sc.dids.alice, commit.cid)
      // Check
      const getGoodPost = agent.api.app.gndr.feed.getPostThread(
      ...
      const getBadPost = agent.api.app.gndr.feed.getPostThread(
  ...
  describe('indexHandle', () => {
    ...
    it('reindexes handle for existing did when forced', async () => {
      ...
      await network.gndr.sub.indexingSvc.indexHandle(did, now, true)
  ...
  describe('deleteActor', () => {
    it('does not unindex actor when they are still being hosted by their pds', async () => {
      const { data: profileBefore } = await agent.api.app.gndr.actor.getProfile(
        { actor: sc.dids.alice },
        {
          headers: await network.serviceHeaders(
            sc.dids.bob,
      ...
      await network.gndr.sub.indexingSvc.deleteActor(sc.dids.alice)
      const { data: profileAfter } = await agent.api.app.gndr.actor.getProfile(
        { actor: sc.dids.alice },
        {
          headers: await network.serviceHeaders(
            sc.dids.bob,
    ...
    it('unindexes actor when they are no longer hosted by their pds', async () => {
      const { alice } = sc.dids
      const getProfileBefore = agent.api.app.gndr.actor.getProfile(
        { actor: alice },
        {
          headers: await network.serviceHeaders(
            sc.dids.bob,
      ...
      // Index tombstone
      await network.gndr.sub.indexingSvc.deleteActor(alice)
      const getProfileAfter = agent.api.app.gndr.actor.getProfile(
        { actor: alice },
        {
          headers: await network.serviceHeaders(
            sc.dids.bob,
