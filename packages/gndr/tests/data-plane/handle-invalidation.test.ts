...
describe('handle invalidation', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_handle_invalidation',
    })
    ...
    const origResolve = network.gndr.dataplane.idResolver.handle.resolve
    network.gndr.dataplane.idResolver.handle.resolve = async (
      handle: string,
    ) => {
  ...
  it('deals with handle contention', async () => {
    ...
    const aliceRes = await agent.api.app.gndr.actor.getProfile(
      { actor: alice },
      {
        headers: await network.serviceHeaders(
          alice,
    ...
    const bobRes = await agent.api.app.gndr.actor.getProfile(
      { actor: bob },
      {
        headers: await network.serviceHeaders(
          alice,
