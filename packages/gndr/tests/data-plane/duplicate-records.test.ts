...
describe('duplicate record', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_duplicates',
    })
    db = network.gndr.db
  ...
  it('dedupes reposts', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      await network.gndr.sub.indexingSvc.indexRecord(
        uri,
        cid,
        repost,
    ...
    await network.gndr.sub.indexingSvc.deleteRecord(uris[0], false)

    count = await countRecords(db, 'repost')
    expect(count).toBe(1)

    await network.gndr.sub.indexingSvc.deleteRecord(uris[1], true)
