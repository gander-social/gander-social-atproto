...
describe('thread mutes', () => {
  ...
  beforeAll(async () => {
    network = await TestNetwork.create({
      dbPostgresSchema: 'gndr_thread_mutes',
    })
  ...
  it('mutes threads', async () => {
    await agent.api.app.gndr.graph.muteThread(
  ...
  it('prevents notifs from replies', async () => {
    ...
    const notifsRes = await agent.api.app.gndr.notification.listNotifications(
      {},
      {
        headers: await network.serviceHeaders(
          alice,
  ...
  it('unmutes threads', async () => {
    await agent.api.app.gndr.graph.unmuteThread(
      { root: rootPost.uriStr },
      {
        encoding: 'application/json',
        headers: await network.serviceHeaders(
