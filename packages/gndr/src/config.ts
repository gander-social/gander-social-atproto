...
export class ServerConfig {
  ...
  static readEnv(overrides?: Partial<ServerConfigValues>) {
    const version = process.env.GNDR_VERSION || undefined
    const debugMode =
      // Because security related features are disabled in development mode, this requires explicit opt-in.
      process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'test'
    const publicUrl = process.env.GNDR_PUBLIC_URL || undefined
    const serverDid = process.env.GNDR_SERVER_DID || 'did:example:test'
    const envPort = parseInt(process.env.GNDR_PORT || '', 10)
    const port = isNaN(envPort) ? 2584 : envPort
    const didPlcUrl = process.env.GNDR_DID_PLC_URL || 'http://localhost:2582'
    const alternateAudienceDids = envList(process.env.GNDR_ALT_AUDIENCE_DIDS)
    const entrywayJwtPublicKeyHex =
      process.env.GNDR_ENTRYWAY_JWT_PUBLIC_KEY_HEX || undefined
    let liveNowConfig: LiveNowConfig | undefined
    if (process.env.GNDR_LIVE_NOW_CONFIG) {
      try {
        const parsed = JSON.parse(process.env.GNDR_LIVE_NOW_CONFIG)
        ...
        log.error({ err }, 'Invalid GNDR_LIVE_NOW_CONFIG')
      }
    }
    const handleResolveNameservers = envList(
      process.env.GNDR_HANDLE_RESOLVE_NAMESERVERS,
    )
    const cdnUrl = process.env.GNDR_CDN_URL || process.env.GNDR_IMG_URI_ENDPOINT
    const etcdHosts =
      overrides?.etcdHosts ?? envList(process.env.GNDR_ETCD_HOSTS)
    // e.g. https://video.invalid/watch/%s/%s/playlist.m3u8
    const videoPlaylistUrlPattern = process.env.GNDR_VIDEO_PLAYLIST_URL_PATTERN
    // e.g. https://video.invalid/watch/%s/%s/thumbnail.jpg
    const videoThumbnailUrlPattern =
      process.env.GNDR_VIDEO_THUMBNAIL_URL_PATTERN
    const blobCacheLocation = process.env.GNDR_BLOB_CACHE_LOC
    const searchUrl =
      process.env.GNDR_SEARCH_URL ||
      process.env.GNDR_SEARCH_ENDPOINT ||
      undefined
    const suggestionsUrl = process.env.GNDR_SUGGESTIONS_URL || undefined
    const suggestionsApiKey = process.env.GNDR_SUGGESTIONS_API_KEY || undefined
    const topicsUrl = process.env.GNDR_TOPICS_URL || undefined
    const topicsApiKey = process.env.GNDR_TOPICS_API_KEY
    const dataplaneUrls =
      overrides?.dataplaneUrls ?? envList(process.env.GNDR_DATAPLANE_URLS)
    const dataplaneUrlsEtcdKeyPrefix =
      process.env.GNDR_DATAPLANE_URLS_ETCD_KEY_PREFIX || undefined
    const dataplaneHttpVersion = process.env.GNDR_DATAPLANE_HTTP_VERSION || '2'
    const dataplaneIgnoreBadTls =
      process.env.GNDR_DATAPLANE_IGNORE_BAD_TLS === 'true'
    assert(
    ...
    const labelsFromIssuerDids = envList(
      process.env.GNDR_LABELS_FROM_ISSUER_DIDS,
    )
    const bsyncUrl = process.env.GNDR_BSYNC_URL || undefined
    assert(bsyncUrl)
    const bsyncApiKey = process.env.GNDR_BSYNC_API_KEY || undefined
    const bsyncHttpVersion = process.env.GNDR_BSYNC_HTTP_VERSION || '2'
    const bsyncIgnoreBadTls = process.env.GNDR_BSYNC_IGNORE_BAD_TLS === 'true'
    assert(bsyncHttpVersion === '1.1' || bsyncHttpVersion === '2')
    const courierUrl = process.env.GNDR_COURIER_URL || undefined
    const courierApiKey = process.env.GNDR_COURIER_API_KEY || undefined
    const courierHttpVersion = process.env.GNDR_COURIER_HTTP_VERSION || '2'
    const courierIgnoreBadTls =
      process.env.GNDR_COURIER_IGNORE_BAD_TLS === 'true'
    assert(courierHttpVersion === '1.1' || courierHttpVersion === '2')
    const blobRateLimitBypassKey =
      process.env.GNDR_BLOB_RATE_LIMIT_BYPASS_KEY || undefined
    ...
    const blobRateLimitBypassHostname =
      process.env.GNDR_BLOB_RATE_LIMIT_BYPASS_HOSTNAME || undefined
    assert(
    ...
    const adminPasswords = envList(
      process.env.GNDR_ADMIN_PASSWORDS || process.env.GNDR_ADMIN_PASSWORD,
    )
    ...
    const statsigKey =
      process.env.NODE_ENV === 'test'
        ? 'secret-key'
        : process.env.GNDR_STATSIG_KEY || undefined
    const statsigEnv =
      process.env.NODE_ENV === 'test'
        ? 'test'
        : process.env.GNDR_STATSIG_ENV || 'development'
    const clientCheckEmailConfirmed =
      process.env.GNDR_CLIENT_CHECK_EMAIL_CONFIRMED === 'true'
    const topicsEnabled = process.env.GNDR_TOPICS_ENABLED === 'true'
    const indexedAtEpoch = process.env.GNDR_INDEXED_AT_EPOCH
      ? new Date(process.env.GNDR_INDEXED_AT_EPOCH)
      : undefined
    assert(
      !indexedAtEpoch || !isNaN(indexedAtEpoch.getTime()),
      'invalid GNDR_INDEXED_AT_EPOCH',
    )
    const bigThreadUris = new Set(envList(process.env.GNDR_BIG_THREAD_URIS))
    const bigThreadDepth = process.env.GNDR_BIG_THREAD_DEPTH
      ? parseInt(process.env.GNDR_BIG_THREAD_DEPTH || '', 10)
      : undefined
    const maxThreadDepth = process.env.GNDR_MAX_THREAD_DEPTH
      ? parseInt(process.env.GNDR_MAX_THREAD_DEPTH || '', 10)
      : undefined
    const maxThreadParents = process.env.GNDR_MAX_THREAD_PARENTS
      ? parseInt(process.env.GNDR_MAX_THREAD_PARENTS || '', 10)
      : 50
    const threadTagsHide = new Set(envList(process.env.GNDR_THREAD_TAGS_HIDE))
    const threadTagsBumpDown = new Set(
      envList(process.env.GNDR_THREAD_TAGS_BUMP_DOWN),
    )

    const notificationsDelayMs = process.env.GNDR_NOTIFICATIONS_DELAY_MS
      ? parseInt(process.env.GNDR_NOTIFICATIONS_DELAY_MS || '', 10)
      : 0

    const disableSsrfProtection = process.env.GNDR_DISABLE_SSRF_PROTECTION
      ? process.env.GNDR_DISABLE_SSRF_PROTECTION === 'true'
      : debugMode

    const proxyAllowHTTP2 = process.env.GNDR_PROXY_ALLOW_HTTP2 === 'true'
    const proxyHeadersTimeout =
      parseInt(process.env.GNDR_PROXY_HEADERS_TIMEOUT || '', 10) || undefined
    const proxyBodyTimeout =
      parseInt(process.env.GNDR_PROXY_BODY_TIMEOUT || '', 10) || undefined
    const proxyMaxResponseSize =
      parseInt(process.env.GNDR_PROXY_MAX_RESPONSE_SIZE || '', 10) || undefined
    const proxyMaxRetries =
      parseInt(process.env.GNDR_PROXY_MAX_RETRIES || '', 10) || undefined
    const proxyPreferCompressed =
      process.env.GNDR_PROXY_PREFER_COMPRESSED === 'true'
