...
import { isSkeletonReasonRepost } from '../../../../lexicon/types/app/gndr/feed/defs'
import { QueryParams as GetFeedParams } from '../../../../lexicon/types/app/gndr/feed/getFeed'
import { OutputSchema as SkeletonOutput } from '../../../../lexicon/types/app/gndr/feed/getFeedSkeleton'
import {
  HydrationFnInput,
  PresentationFnInput,
  RulesFnInput,
...
import { GNDR_USER_AGENT, resHeaders } from '../../../util'
  ...
  server.app.gndr.feed.getFeed({
    ...
    handler: async ({ params, auth, req }) => {
      ...
      const headers = noUndefinedVals({
        'user-agent': GNDR_USER_AGENT,
        authorization: req.headers['authorization'],
        'accept-language': req.headers['accept-language'],
        'x-gndr-topics': Array.isArray(req.headers['x-gndr-topics'])
          ? req.headers['x-gndr-topics'].join(',')
          : req.headers['x-gndr-topics'],
      })
...
const skeletonFromFeedGen = async (
  ...
  try {
    ...
    const result = await agent.api.app.gndr.feed.getFeedSkeleton(
      {
        feed: params.feed,
