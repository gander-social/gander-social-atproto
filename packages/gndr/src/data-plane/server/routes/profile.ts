...
export default (db: Database): Partial<ServiceImpl<typeof Service>> => ({
  async getActors(req) {
    ...
    const profileUris = dids.map(
      (did) => `at://${did}/app.gndr.actor.profile/self`,
    )
    const statusUris = dids.map(
      (did) => `at://${did}/app.gndr.actor.status/self`,
    )
    const chatDeclarationUris = dids.map(
      (did) => `at://${did}/chat.gndr.actor.declaration/self`,
    )
    const notifDeclarationUris = dids.map(
      (did) => `at://${did}/app.gndr.notification.declaration/self`,
    )
